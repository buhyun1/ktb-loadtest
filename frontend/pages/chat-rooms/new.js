...
function NewChatRoom() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('방 생성 API 응답:', result);
      if (!result.data) {
        setError('방 생성 응답에 data 필드가 없습니다: ' + JSON.stringify(result));
        return;
      }
      if (!result.data._id) {
        setError('방 생성 응답에 _id 필드가 없습니다: ' + JSON.stringify(result.data));
        return;
      }
      // 생성된 채팅방에 자동으로 입장
      await joinRoom(result.data._id, formData.hasPassword ? formData.password : undefined);
